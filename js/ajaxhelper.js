//  This file provided useful functions for making Ajax calls
// $Id: ajaxhelper.js,v 1.5 2013/05/14 04:54:33 cvs Exp $
...
function makeJSONAjaxCall(ws_url, parameters, async, callback, errcallback) {
	// TODO merge with makeGenericAjaxCall, actually use it
 try {
   var xmlhttp = getXmlHttp();
   if (parameters) {
      xmlhttp.open("POST",ws_url,async); // (method,url,async,usr,pswd)
...
function makeIFrameAjaxCall(ws_url, callback, errorCalBack) {
    ...
    var five = (navigator.appVersion.indexOf('5.0') >= 0);
